...
//!
//! Everything that decides what happens lives in [`amoeba_rl::sim`]. This
//! file only translates between the window and the sim's logical world; the
//! sim's [`Cue`](amoeba_rl::sim::Cue)s become sound over in [`audio`].
...
use amoeba_rl::sim::{InputFrame, Sim, Vec2, WORLD_H, WORLD_W};
